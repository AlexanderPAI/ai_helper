...
- интерактивный статус с текущим этапом работы и прошедшим временем;
...
│   ├── progress.py           # независимые от транспорта события прогресса
...
│   ├── progress.py           # понятные статусы этапов и прошедшее время
...
статус показывает понятный пользователю текущий этап и прошедшее время. Например:
...
При переходе к следующему шагу текст меняется сразу, а раз в 5 секунд обновляется
счётчик времени. В Telegram не выводятся имена инструментов, их аргументы,
служебные идентификаторы и внутренние рассуждения. Когда ответ готов, статусное
сообщение заменяется финальным результатом.
